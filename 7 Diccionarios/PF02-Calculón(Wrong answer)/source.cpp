...
/* ======================================================================
* Implementación de la clase MapTree para representar diccionarios
...
// Función que lee un programa de la entrada y devuelve su AST.
...
int evaluar(const BinTree<string>& a, const MapTree<string, int>& mapa) {
	if (a.root() == "+")
		return evaluar(a.left(), mapa) + evaluar(a.right(), mapa);
	else if (a.root() == "*")
		return evaluar(a.left(), mapa) * evaluar(a.right(), mapa);
	else if (a.root() == "-")
		return evaluar(a.left(), mapa) * evaluar(a.right(), mapa);
	else if (isdigit(a.root()[0]))
		return stoi(a.root());
	else
		return mapa.at(a.root());
...
bool tratar_caso() {
	...
	MapTree<string, int> mapa;
	for (int i = 0; i < n; i++) {
		BinTree<string> a = parse(cin);
		string var = a.left().root();
		int result = evaluar(a.right(), mapa);
		mapa[var] = result;
	}
