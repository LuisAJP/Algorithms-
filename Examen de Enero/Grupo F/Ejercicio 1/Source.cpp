...
P={ N>=0 && long(v)=N}
fun (v:vector<int>, int N) return r:int
Q={ max p,q: 0<=p<q < N && consecutivos(v,p,q): q-p}
...
El coste es lineal O(n) , donde n es el numero de elementos, en el caso peor tiene que recorrer todos los elementos del vector para buscar
la mayor secuencia de numeros consecutivos
